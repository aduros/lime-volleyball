...
all: src/images.h build/cart.wasm

src/images.h: $(wildcard src/*.png)
	@w4 png2src --c $^ > $@
...
bundle: clean build/cart.wasm
	w4 bundle build/cart.wasm --html build/index.html --windows build/lime-volleyball-windows.exe --linux build/lime-volleyball-linux --mac build/lime-volleyball-mac
